...
function verifyPasswordAgainstPeppers({ password, username, storedHash }) {
  ...
  // If DB contains legacy/plaintext password, or otherwise not a SHA-256 hex digest,
  // we cannot use constant-time compare against it.
  if (!isSha256Hex(stored)) return { ok: false, matchedPepper: null };

  const storedBuf = Buffer.from(stored, "hex"); // 32 bytes
  for (const pepper of PEPPER_CANDIDATES) {
    const salt = deriveSaltFromUsername(username);
    const candidateBuf = sha256Buffer(`${String(password)}:${salt}:${String(pepper)}`);
